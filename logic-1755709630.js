...
const CURRENT_INSTANCE = '2025-FA-01';
const CURRENT_TERM_START = { month: 8, day: 25, year: 2025 };
...
wrapper.className = 'nametagWrapper';
...
const row = rows[i]
const numElt = row.querySelector('.line-number')
if (numElt) {
setText(numElt, String(i+1))
}
}
}
}
...
const changing = !this.decoration || value !== this.truthvalue;
...
ref.child('span', {'class': 'rule'}, item).cap(span => this.installRule(span, item));
}
else {
...
(new ElementRef(this.elt))
.child('div')
.child('span', 'Done', {class: 'main-button'})
.cap(span => {new Button(span, handler)})
}
show(step)
{
...
{
if (this.currentItem) {
this.currentItem.className = 'rule';
this.currentItem = null;
}
if (rule && this.rulemap[rule]) {
this.currentItem = this.rulemap[rule];
this.currentItem.className = 'rule selected';
}
}
hide()
{
...
.sibling('div', '<', {class: 'med-button narrow'}).cap(elts, 'prev')
.sibling('div', '>', {class: 'med-button narrow'}).cap(elts, 'next');
...
(new ElementRef(tab))
.child('div')
.child('div', c.title, {class: 'main-button'}).cap(elts, 'actionbtn')
